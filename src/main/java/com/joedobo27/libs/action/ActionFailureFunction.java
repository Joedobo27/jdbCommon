...
public class ActionFailureFunction {
    ...
    public ActionFailureFunction(String name, Function<ActionMaster, Boolean> function) {
        this.name = name;
    ...
    private static void initializeFailureFunctions() {
                ...
                (ActionMaster actionMaster) -> {
                    if (actionMaster.getPerformer().getStatus().getStamina() < actionMaster.getMinimumStamina()) {
                        actionMaster.getPerformer().getCommunicator().sendNormalServerMessage(
                                "You don't have enough stamina to " + actionMaster.action.getActionString() + ".");
                        return true;
                    }
                    return false;
                }));
        failureFunctions.put(2, new ActionFailureFunction("FAILURE_FUNCTION_SERVER_BOARDER_TOO_CLOSE",
                (ActionMaster actionMaster) -> {
                    if (actionMaster.getTargetTile().x < 0 ||
                            actionMaster.getTargetTile().x > 1 << Constants.meshSize ||
                            actionMaster.getTargetTile().y < 0 || actionMaster.getTargetTile().y > 1 << Constants.meshSize) {
                        actionMaster.getPerformer().getCommunicator().sendNormalServerMessage(
                                "You can't " + actionMaster.action.getActionString() + " this close to the server boarder.");
                        return true;
                    }
                    return false;
                }));
        failureFunctions.put(3, new ActionFailureFunction("FAILURE_FUNCTION_GOD_PROTECTED",
                (ActionMaster actionMaster) -> {
                    if (Zones.isTileProtected(actionMaster.getTargetTile().x, actionMaster.getTargetTile().y)) {
                        actionMaster.getPerformer().getCommunicator().sendNormalServerMessage("" +
                                "This tile is protected by the gods. You can not " + actionMaster.action.getActionString() + " here.");
                        return true;
                    }
                    return false;
                }));
        failureFunctions.put(4, new ActionFailureFunction("FAILURE_FUNCTION_PVE_VILLAGE_ENEMY_ACTION",
                actionMaster -> {
                    ...
                    if (village != null && !village.isActionAllowed((short) actionMaster.getActionId(),
                            actionMaster.getPerformer(), false,
        ...
        failureFunctions.put(5, new ActionFailureFunction("FAILURE_FUNCTION_PVP_VILLAGE_ENEMY_ACTION",
                (ActionMaster actionMaster) -> {
                    Village village = Zones.getVillage(actionMaster.getTargetTile().x, actionMaster.getTargetTile().y,
                            actionMaster.getPerformer().isOnSurface());
                    if (village != null && !village.isActionAllowed((short) actionMaster.getActionId(), actionMaster.getPerformer(),
        ...
        failureFunctions.put(6, new ActionFailureFunction("FAILURE_FUNCTION_ROCK_MESH_AND_CAVE_CEILING_TOO_CLOSE",
                (ActionMaster actionMaster) -> {
                    ...
                    if (TileUtilities.getCaveFloorHeight(actionMaster.getTargetTile()) +
                        ...
                        actionMaster.getPerformer().getCommunicator().sendNormalServerMessage("" +
                                "A cave is preventing the " + actionMaster.action.getActionString() + " action.");
                        return true;
                    }
                    return false;
                }));
        failureFunctions.put(7, new ActionFailureFunction("FAILURE_FUNCTION_CORNER_OCCUPIED_BY_FENCE",
                (ActionMaster actionMaster) -> {
                    ...
                    if (IntStream.range(0, fenceTiles.length)
                        ...
                        actionMaster.getPerformer().getCommunicator().sendNormalServerMessage("" +
                                "A "+actionMaster.action.getActionString()+
        ...
        failureFunctions.put(10, new ActionFailureFunction("FAILURE_FUNCTION_IS_OCCUPIED_BY_BRIDGE_EXIT",
                (ActionMaster actionMaster) -> {
                    TilePos[] bridgeExits = {actionMaster.getTargetTile().West(), actionMaster.getTargetTile().North(),
                            actionMaster.getTargetTile().East(), actionMaster.getTargetTile().South()};
        ...
        failureFunctions.put(12, new ActionFailureFunction("FAILURE_FUNCTION_IS_DIGGING_ROCK",
            ...
            if (TileUtilities.getDirtDepth(actionMaster.getTargetTile()) <= 0){
                actionMaster.getPerformer().getCommunicator().sendNormalServerMessage("" +
                        "You can't do "+actionMaster.action.getActionEntry().getVerbString()+
                        " in rock.");
                return true;
            }
                    return false;
        ...
        failureFunctions.put(13, new ActionFailureFunction("FAILURE_FUNCTION_NO_DIRT_NEARBY",
                (ActionMaster actionMaster) -> {
                    ...
                    if ((groundItems == null || groundItems.length == 0) && (inventoryItems == null ||
                        ...
                        actionMaster.getPerformer().getCommunicator().sendNormalServerMessage("" +
                                "There is no dirt nearby to do "+actionMaster.action.getActionEntry().getVerbString()+
                                " with.");
                        return true;
                    }
                    return false;
        ...
        failureFunctions.put(14, new ActionFailureFunction("FAILURE_FUNCTION_NO_CONCRETE_NEARBY",
                (ActionMaster actionMaster) -> {
                    ...
                    if ((groundItems == null || groundItems.length == 0) && (inventoryItems == null ||
                            inventoryItems.length == 0)) {
                        actionMaster.getPerformer().getCommunicator().sendNormalServerMessage("" +
                                "There is no concrete nearby to do "+actionMaster.action.getActionEntry().getVerbString()+
                                " with.");
                        return true;
                    }
                    return false;
